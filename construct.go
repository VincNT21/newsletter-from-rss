...
func getNewsletterData(gc GlobalConfig) (Newsletter, error) {
	...
	newsletterData := Newsletter{
		...
		OutroLinkTitle: gc.OutroLinkTitle,
	}
	...
	for _, cat := range gc.Categories {
		...
		if len(cat.SubCategories) == 0 {
			...
			if cat.RssLink != "" {
				items, err := fetchAndSortRssData(cat, gc.DateFormat)
				if err != nil {
			...
			for _, subCat := range cat.SubCategories {
				...
				if subCat.RssLink != "" {
					fetchItems, err := fetchAndSortRssData(subCat, gc.DateFormat)
					if err != nil {
						return newsletterData, err
					}
					subItems = fetchItems
...
func fetchAndSortRssData(cat Category, dateFormat string) ([]NewsletterItems, error) {
	rssLink := cat.RssLink
	// Get the feed items
	...
	return convertItems(rssItems, dateFormat), nil
}
